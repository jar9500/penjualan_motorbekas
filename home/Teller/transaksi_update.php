...
<!-- header -->
<div class="header">
    <h1 class="mt-5">Edit Data Transaksi</h1>
</div>
<!-- body -->
<div class="container" >
		<div class="bg-light m-4 p-4">
            <?php
                include '../../koneksi.php';
                $IdTrsk=$_GET['IdTrsk'];
                $data=mysqli_query($koneksi,"SELECT * from transaksi_penjualan where IdTrsk='$IdTrsk' ") or die(mysqli_error($koneksi));
                $transaksi_penjualan=mysqli_fetch_array($data);
            ?>
			<form method="post" action="../../proses/transaksi_updateproses.php?IdTrsk=<?php echo $IdTrsk?>">
                <div class="form-group row my-4">
                    <label class="col-sm-2 col-form-label" for="IdTrsk">ID Transaksi</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="IdTrsk" value="<?php echo $transaksi_penjualan['IdTrsk']?>" required>
                    </div>
                </div>
                <div class="form-group row my-4">
                    <label class="col-sm-2 col-form-label" for="Tgl_Trsk">Tanggal Transaksi</label>
                    <div class="col-sm-4">
                        <input type="date" class="form-control" name="Tgl_Trsk" value="<?php echo $transaksi_penjualan['Tgl_Trsk']?>" required>
                    </div>
                </div>
                <div class="form-group row my-4">
                    <label class="col-sm-2 col-form-label" for="Id_Cust">ID Customer</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="Id_Cust" value="<?php echo $transaksi_penjualan['Id_Cust']?>" required>
                    </div>
                </div>
                <div class="form-group row my-4">
                    <label class="col-sm-2 col-form-label" for="Id_Kendaraan">ID Kendaraan</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="Id_Kendaraan" value="<?php echo $transaksi_penjualan['Id_Kendaraan']?>" required>
                    </div>
                </div>
                <div class="form-group row my-4">
                    <label class="col-sm-2 col-form-label" for="Harga_Jual">Harga Jual</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="Harga_Jual" value="<?php echo $transaksi_penjualan['Harga_Jual']?>" required>
                    </div>
                </div>
                <div class="form-group row my-4">
                    <label class="col-sm-2 col-form-label" for="Harga_Jual_Real">Harga Jual Asli</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="Harga_Jual_Real" value="<?php echo $transaksi_penjualan['Harga_Jual_Real']?>" required>
                    </div>
                </div>
